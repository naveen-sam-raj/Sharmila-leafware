...
const CATEGORY_MAP: Record<string, string> = {
  ...
  Packing: 'PACKING',
  Warehouse: 'WAREHOUSE',
  'Export Containers': 'EXPORT_CONTAINERS',
};

const DISPLAY_CAT_MAP: Record<string, string> = {
  MANUFACTURING: 'Manufacturing',
...
export default function Gallery() {
  ...
  const activeEnum = CATEGORY_MAP[active] || 'ALL';

  const filtered = active === 'All' || activeEnum === 'ALL'
    ? items
    : items.filter((i) => i.category === activeEnum || i.category === active);

  // Masonry: distribute items across 4 columns
    ...
    <section id="gallery" className="relative py-24 lg:py-32 overflow-hidden bg-[#F5E6C8]">
      <div className="section-padding relative z-10 max-w-7xl mx-auto">
        <SectionHeading
          ...
          title={
            <>
              From Leaf to <span className="gold-text">Legacy</span>
            </>
          }
            ...
            {columns.map((col, ci) => (
              <div key={ci} className="flex flex-col gap-4">
                {col.map((item, i) => {
                  ...
                  const displayCat = DISPLAY_CAT_MAP[item.category] || item.category;
                    ...
                    >
                      <div className={`relative overflow-hidden ${ci % 2 === 0 ? 'aspect-[3/4]' : 'aspect-square'}`}>
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-5 translate-y-3 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-500">
                          <span className="font-sans text-[10px] tracking-[0.2em] text-[#C8A45D] font-bold uppercase">
                            {displayCat}
                          </span>
